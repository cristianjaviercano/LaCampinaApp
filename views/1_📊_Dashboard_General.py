...
if df_det.empty:
    st.warning(
        "No hay datos de compras cargados. Ve al módulo **Gestor Base Maestra → Compras** y carga un Purchase Order."
    )
    st.stop()
...
# Include all states, as "Visitas Fallidas" may be relevant for some reports
...
if len(date_range) == 2:
    ...
    df_prev = df_det[
        (df_det["Fecha"].dt.date >= prev_start) & (df_det["Fecha"].dt.date <= prev_end)
    ]
    if sel_vendedor != "Todos":
...
prev_unids = (
    df_prev["Cantidad"].sum()
    if (not df_prev.empty and "Cantidad" in df_prev.columns)
    else 0
)
...
def render_kpi(col, label, value, delta_clase, delta_text, fmt="$", color="green"):
    color_class = {"green": "", "red": " red", "blue": " blue", "amber": " amber"}[
        color
    ]
    if fmt == "$":
        val_str = f"${value:,.0f}"
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">📈 Progreso y Tendencia</div>', unsafe_allow_html=True
)
c_gauge, c_trend = st.columns([1, 2])

with c_gauge:
    gauge_color = (
        "#2e7d32" if pct_meta >= 80 else ("#e65100" if pct_meta >= 50 else "#c62828")
    )
    fig_gauge = go.Figure(
        go.Indicator(
            ...
            domain={"x": [0, 1], "y": [0, 1]},
            title={
                "text": f"Meta: ${meta_ventas:,.0f}",
                "font": {"size": 13, "color": "#555"},
            },
            delta={
                "reference": meta_ventas,
            ...
            gauge={
                ...
                "steps": [
                    ...
                    {
                        "range": [meta_ventas * 0.5, meta_ventas * 0.8],
                        "color": "#fff9c4",
                    },
    ...
    fig_gauge.add_annotation(
        x=0.5,
        y=0.18,
        text=f"<b>{pct_meta:.1f}% de la meta</b>",
        showarrow=False,
        font=dict(size=13, color=gauge_color),
        xref="paper",
        yref="paper",
    )
...
with c_trend:
    ventas_dia = (
        df.groupby(df["Fecha"].dt.date)["Total"]
        .sum()
        .reset_index()
    ...
    if len(ventas_dia) >= 3:
        ventas_dia["MA7"] = (
            ventas_dia["Ventas"]
            .rolling(window=min(7, len(ventas_dia)), min_periods=1)
            .mean()
        )
        fig_trend.add_trace(
            go.Scatter(
    ...
    fig_trend.add_hline(
        ...
        line_color="#e53935",
        line_width=1.5,
        annotation_text=f"Meta diaria ${meta_dia:,.0f}",
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">📆 Comportamiento Temporal y Comercial</div>',
    unsafe_allow_html=True,
)
...
with c_heat:
    ...
    # Pivot para heatmap
    heat_data = df.groupby(["DiaSemana", "AnoSem"])["Total"].sum().reset_index()
    dia_order = ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"]
    heat_pivot = heat_data.pivot(
        index="DiaSemana", columns="AnoSem", values="Total"
    ).fillna(0)
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">🧩 Desglose de Ventas</div>', unsafe_allow_html=True
)
...
with c_pay:
    if "MetodoPago" in df.columns:
        pago_data = (
            df.groupby("NoPedido")
            .agg(MetodoPago=("MetodoPago", "first"), Total=("Total", "sum"))
            .groupby("MetodoPago")["Total"]
            .sum()
            .reset_index()
        )
        fig_pay = px.pie(
        ...
        fig_pay.update_layout(
            height=300, margin=dict(l=5, r=5, t=40, b=5), showlegend=False
        )
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="section-title">📅 Agrupación Temporal</div>', unsafe_allow_html=True
)
...
with tab_ruta:
    if "Ruta" in df.columns and df["Ruta"].notna().any():
        ...
        fig_ruta = px.bar(
            ...
            template="plotly_white",
            title="Recaudo por Ruta de Visita",
        )
...
with c_estado:
    if "Estado" in df.columns and df["Estado"].notna().any():
        estado_data = (
            df.groupby("NoPedido")
            .agg(Estado=("Estado", "first"), Total=("Total", "sum"))
        ...
        fig_est.update_layout(
            height=280, margin=dict(l=5, r=5, t=40, b=5), showlegend=False
        )
        ...
        st.metric(
            "Días en período",
            (date_range[1] - date_range[0]).days + 1 if len(date_range) == 2 else "—",
        )
        st.metric(
            "Promedio Diario",
            f"${total_ventas / max((date_range[1] - date_range[0]).days + 1, 1):,.0f}"
            if len(date_range) == 2
            else "—",
        )
        st.metric(
            "Máx. venta en un día",
            f"${df.groupby(df['Fecha'].dt.date)['Total'].sum().max():,.0f}",
        )
...
with c_tabla:
    resumen_vend = (
        ...
        .agg(
            ...
            Ticket_Prom=(
                "Total",
                lambda x: (
                    x.sum() / df.loc[x.index, "NoPedido"].nunique()
                    if df.loc[x.index, "NoPedido"].nunique() > 0
                    else 0
                ),
            ),
        )
    ...
    resumen_vend["% del Total"] = (
        resumen_vend["Recaudo"] / resumen_vend["Recaudo"].sum() * 100
    )

    st.markdown("**Desempeño por Vendedor en el Período**")
    st.dataframe(
        resumen_vend.style.format(
            {
                "Recaudo": "${:,.0f}",
                "Ticket_Prom": "${:,.0f}",
                "Unidades": "{:,.0f}",
