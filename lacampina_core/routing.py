...
_BASE_DIR     = _CORE_DIR.parent / "datos_maestros"

# Validar si _BASE_DIR tiene los archivos, de lo contrario buscar relativo al espacio de trabajo
if not (_BASE_DIR / "sahagun_drive.pkl").exists() and not (_BASE_DIR / "sahagun_drive.graphml").exists():
    fallback_dir = _CORE_DIR.parent.parent / "APP_Lacampiña2.0" / "datos_maestros"
    if fallback_dir.exists():
        _BASE_DIR = fallback_dir

_PKL_PATH     = _BASE_DIR / "sahagun_drive.pkl"       # Binario Python  ← prioridad
...
def get_graph():
    ...
    Usa st.cache_resource si Streamlit está disponible.
    """
    try:
    ...
    # Definir cargadores con o sin caché según disponibilidad
    if st is not None:
        ...
        @st.cache_resource(show_spinner="🗺️ Cargando red vial de Sahagún...")
        def _desde_graphml():
            import osmnx as ox
