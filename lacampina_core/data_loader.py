...
# Intentar importar streamlit para soporte de caché en dashboards web
try:
    import streamlit as st
    cache_data_decorator = st.cache_data
except ImportError:
    st = None
    # Decorador dummy si streamlit no está disponible (ej. en la app Tkinter)
...
# Validar si BASE_DIR tiene datos_maestros, de lo contrario buscar relativo al espacio de trabajo
if not (BASE_DIR / "datos_maestros").exists():
    fallback_dir = _CORE_DIR.parent.parent / "APP_Lacampiña2.0"
    if (fallback_dir / "datos_maestros").exists():
...
@cache_data_decorator(ttl=None)  # sin expiración — solo se limpia con "Refrescar Caché"
def load_maestros():
    """
    ...
    try:
        ...
        _log_error(f"Error cargando datos maestros: {e}")
        return None
...
def load_compras(fecha_inicio=None, fecha_fin=None):
    ...
    if compras_path.exists():
        try:
            ...
            _log_error(f"Error cargando compras_maestras.json: {e}")
    ...
    try:
        ...
        _log_error(f"Error uniendo y procesando la macro-base de compras: {e}")
        return None, None
...
def load_data():
    ...
    # Merge con clientes para asegurar ClienteNombre sea el nombre completo de los maestros
