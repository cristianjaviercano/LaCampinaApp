...
def load_maestros():
    ...
    try:
        ...
        df_ubicaciones = pd.read_json(ubicaciones_path) if ubicaciones_path.exists() else pd.DataFrame()
        df_vendedores  = pd.read_json(vendedores_path)  if vendedores_path.exists()  else pd.DataFrame()
        df_productos   = pd.read_json(productos_path)   if productos_path.exists()   else pd.DataFrame()
        df_clientes    = pd.read_json(clientes_path)    if clientes_path.exists()    else pd.DataFrame()
        ...
        if not df_ubicaciones.empty and 'Ciudad' in df_ubicaciones.columns:
            ...
            df_ubicaciones['Ciudad'] = df_ubicaciones['Ciudad'].str.replace('Á', 'A').str.replace('É', 'E').str.replace('Í', 'I').str.replace('Ó', 'O').str.replace('Ú', 'U')


        if not df_clientes.empty and 'Ciudad' in df_clientes.columns:
            df_clientes['Ciudad'] = df_clientes['Ciudad'].str.upper().str.strip()
            df_clientes['Ciudad'] = df_clientes['Ciudad'].str.replace('Á', 'A').str.replace('É', 'E').str.replace('Í', 'I').str.replace('Ó', 'O').str.replace('Ú', 'U')
...
def load_compras(fecha_inicio=None, fecha_fin=None):
                    ...
                    try:
                        ...
                        if not df_det.empty:
                            ...
                            if 'ClienteNombre' not in df_det.columns:
                                df_det['ClienteNombre'] = "Cliente " + df_det['ClienteCodigo'].astype(str)
                        ...
                        st.warning(f"Error leyendo el lote histórico {batch_dir.name}: {e}")
                        
    if not dfs:
        st.warning("No hay datos de compras cargados. Ve a 'Carga de Datos' o al Gestor Maestro para subir la información.")
        return None, None
    ...
    try:
        ...
        if 'Ciudad' in df.columns:
            ...
            df['Ciudad'] = df['Ciudad'].str.replace('Á', 'A').str.replace('É', 'E').str.replace('Í', 'I').str.replace('Ó', 'O').str.replace('Ú', 'U')
        ...
        df_compras = (
            df.groupby(["NoPedido", "Fecha", "ClienteCodigo", "ClienteNombre",
                        "Barrio", "Ciudad", "Ruta", "CodigoVendedor",
                        "Vendedor", "MetodoPago", "Estado"], dropna=False)
